...
export const generateDailyJobReportPDF = (profile: UserProfile, data: DailyJobReportData, template: 'minimal' | 'bordered' | 'modern'): Promise<void> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!contentElement) {
        return reject(new Error('PDF render content element not found'));
      }

      const renderHtmlCallback = () => {
        addFooter();
        doc.save(`DailyReport-${data.reportNumber}-${data.clientName}.pdf`);
        resolve();
      };
...
export const generateNotePDF = (profile: UserProfile, job: Job, data: NoteData): Promise<void> => {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (contentElement) {
        ...
        reject(new Error('PDF render content element not found'));
      }
    } catch(e) {
      reject(e);
    }
