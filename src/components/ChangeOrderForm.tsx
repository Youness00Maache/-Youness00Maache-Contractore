    ...
    const renderPageTwo = () => (
        <Card className="max-w-3xl mx-auto w-full animate-fade-in-down">
            ...
            <CardContent className="space-y-6">
                ...
                <div className="pt-4">
                    <Label className="mb-2 block">Your Signature (Contractor)</Label>
                    <div className="mt-1 border rounded-md overflow-hidden">
                        <SignaturePad onSave={(url) => setFormData(prev => ({ ...prev, signatureUrl: url }))} initialDataUrl={formData.signatureUrl} />
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
                        Add your signature to authorize this change order.
                    </p>
                </div>

                {/* Client Signature (Read-only if signed via approval) */}
                {formData.clientSignatureUrl && (
                    <div className="pt-4 border-t border-border mt-4">
                        <Label className="mb-2 block flex items-center gap-2">
                            <CheckCircleIcon className="w-4 h-4 text-green-600" />
                            Client Signature (Approved Digitally)
                        </Label>
                        <div className="border rounded-md p-4 bg-green-50 dark:bg-green-950/20">
                            <img src={formData.clientSignatureUrl} alt="Client Signature" className="h-24 border-b border-green-200 pb-2 mb-2" />
                            <p className="text-xs text-green-700 dark:text-green-400 font-medium">
                                ✓ Signed by {formData.clientSignedBy || formData.clientName} on {formData.clientSignedAt ? new Date(formData.clientSignedAt).toLocaleString() : 'N/A'}
                            </p>
                        </div>
                    </div>
                )}
