...
const ProfitCalculatorView: React.FC<ProfitCalculatorViewProps> = ({ onBack, profile }) => {
    ...
    const handleExportPdf = async () => {
        ...
        // Construct a fake Estimate object to reuse the generator
        const total = totalCharge; // Pre-tax total usually

        const fakeData = {
            estimateNumber: 'PROFIT-REPORT',
            date: new Date().toISOString(),
            clientName: 'Internal Calculation',
            clientAddress: '',
            clientEmail: '',
            clientPhone: '',
            projectName: calcName || 'Profit Calculation',
            ...
            notes: `Calculated Profit: $${profit.toFixed(2)} (${margin.toFixed(1)}% Margin)\nTotal Cost: $${totalCost.toFixed(2)}\n\n(This is an internal profit report)`,
        ...
        try {
            const base64 = await generateDocumentBase64(FormType.Estimate, fakeData, profile, fakeJob as any);
            if (base64) {
