...
import type { InvoiceData, LineItem, Job, UserProfile, SavedItem } from '../types.ts';
import { generateInvoicePDF } from '../services/pdfGenerator.ts';
...
const defaultInvoice: Omit<InvoiceData, 'clientName' | 'clientAddress' | 'companyName' | 'companyAddress' | 'companyPhone' | 'companyWebsite' | 'logoUrl'> = {
  ...
  lineItems: [{ id: crypto.randomUUID(), description: '', quantity: 1, rate: 0, progressValue: 0, progressPercentage: 0 }],
  taxRate: 0,
  discount: 0,
  shipping: 0,
...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ job, userProfile, invoice, onSave, onClose, onUploadImage, savedItems = [] }) => {
  ...
  const addLineItem = () => {
    setInvoiceData(prev => ({
      ...prev,
      lineItems: [...prev.lineItems, { id: crypto.randomUUID(), description: '', quantity: 1, rate: 0, progressValue: 0, progressPercentage: 0 }],
    }));
  };
  ...
  const calculateSubtotal = () => {
    return invoiceData.lineItems.reduce((acc, item) => {
        if (invoiceData.isProgressBilling) {
            const val = Number(item.progressValue || 0);
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-12 gap-2 text-sm font-medium text-muted-foreground px-2">
            ...
            {invoiceData.isProgressBilling ? (
                <>
                    <div className="col-span-3">Scheduled Value ($)</div>
              ...
              {invoiceData.isProgressBilling ? (
                  <>
                    <div className="col-span-3">
                        ...
                        onChange={(e) => handleLineItemChange(item.id, 'progressValue', parseFloat(e.target.value) || 0)}
                        />
                    </div>
                    <div className="col-span-2">
                        ...
                        onChange={(e) => handleLineItemChange(item.id, 'progressPercentage', Math.min(100, Math.max(0, parseFloat(e.target.value) || 0)))}
                        />
                    </div>
                    <div className="col-span-2">
                        <div className="flex items-center justify-end h-10 px-3 py-2 text-sm font-medium">
                        ${((Number(item.progressValue) || 0) * ((Number(item.progressPercentage) || 0) / 100)).toFixed(2)}
                        </div>
                    </div>
                  </>
              ) : (
