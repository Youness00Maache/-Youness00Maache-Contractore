...
const DailyJobReportForm: React.FC<DailyJobReportFormProps> = ({ profile, job, clients, report, onSave, onBack, onUploadImage }) => {
    ...
    const renderPageOne = () => (
        <Card className="animate-fade-in-down shadow-xl rounded-2xl border-blue-100 dark:border-border overflow-hidden">
            <CardHeader className="bg-gradient-to-r from-blue-50/80 to-indigo-50/50 dark:from-secondary/30 dark:to-secondary/10 border-b border-blue-100 dark:border-border pb-5">
                <CardTitle className="text-xl tracking-tight">Daily Report Setup</CardTitle>
                <CardDescription className="text-sm">Review and confirm the details for this report.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6 p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                        <h3 className="text-sm font-semibold uppercase tracking-wider text-blue-600 dark:text-primary border-b border-blue-100 dark:border-border pb-2">Your Company</h3>
                        <div className="flex flex-col space-y-1.5"><Label htmlFor="companyName">Company Name</Label><Input id="companyName" name="companyName" value={data.companyName} onChange={handleChange} /></div>
                        <div className="flex flex-col space-y-1.5"><Label htmlFor="companyAddress">Address</Label><Input id="companyAddress" name="companyAddress" value={data.companyAddress} onChange={handleChange} /></div>
                        <div className="flex flex-col space-y-1.5"><Label htmlFor="companyPhone">Phone</Label><Input id="companyPhone" name="companyPhone" value={data.companyPhone} onChange={handleChange} /></div>
                        <div className="flex flex-col space-y-1.5"><Label htmlFor="companyWebsite">Website</Label><Input id="companyWebsite" name="companyWebsite" value={data.companyWebsite} onChange={handleChange} /></div>
                    </div>
                    <div className="space-y-4">
                        <h3 className="text-sm font-semibold uppercase tracking-wider text-blue-600 dark:text-primary border-b border-blue-100 dark:border-border pb-2">Project & Client</h3>
                        <div className="flex flex-col space-y-1.5">
                            <Label htmlFor="clientSelect">Select Saved Client</Label>
                            <select
                                id="clientSelect"
                                className="flex h-10 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-colors"
                ...
                <div className="pt-4">
                    <h3 className="text-sm font-semibold uppercase tracking-wider text-blue-600 dark:text-primary border-b border-blue-100 dark:border-border pb-2 mb-4">Report Details & Logo</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-4">
            ...
            <CardFooter className="flex justify-end space-x-2 bg-muted/20 rounded-b-2xl border-t border-border py-4">
                <Button variant="outline" onClick={onBack}>Cancel</Button>
                <Button onClick={() => setPage(2)} className="bg-blue-600 hover:bg-blue-700 text-white shadow-sm shadow-blue-500/20">Next</Button>
            </CardFooter>
        </Card>
    );
        ...
        `}</style>
                <div className="flex flex-col h-full relative">
                    ...
                    <div
                        ...
                        className="editor-content w-full p-4 rounded-xl bg-input border border-border/70 focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 focus:outline-none min-h-[300px] transition-colors"
                        onKeyUp={checkActiveState}
                        onMouseUp={checkActiveState}
                    >
                    </div>
                    {resizerState && (
                        <div style={{ position: 'absolute', top: resizerState.top, left: resizerState.left, width: resizerState.width, height: resizerState.height, border: '2px solid var(--primary)', pointerEvents: 'none', boxSizing: 'border-box' }}>
                            <div data-resize-handle="bottom-right" onMouseDown={handleResizeMouseDown} style={{ position: 'absolute', bottom: -6, right: -6, width: 12, height: 12, backgroundColor: 'var(--card)', border: '2px solid var(--primary)', borderRadius: '50%', cursor: 'se-resize', pointerEvents: 'auto' }} />
                        </div>
                    )}
